% This code is written for calculating the harmonic components of the flux
% taken from the 3D magnetostatic model. Then, by using this harmonic
% components, induced voltage is calculated. Also, flux per pole is
...
filename="C:\Users\samet\Documents\GitHub\Studies\3D_2D_Calibration\Codes\Data\fpp3d.csv"; % data path
dataLines= [8 8]; % 8 is the row number, i.e., it can be changed for different field current values
data= importfile(filename, dataLines);

Flux=data(2:end); % taking the flux values; first component of the row is field current
