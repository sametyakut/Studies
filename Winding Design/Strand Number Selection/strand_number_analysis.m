...
total_height=63.4e-3; % in m % slot copper height for single coil without main isolation
total_width=21.6e-3;    % in m % total copper width with isolation
numberOfhorizontalStrand=2; % number of  horizontal strands 
width_strand=7.41e-3; % in m % strand width 
ro=2.0968e-8;         % in Ohm*m  % resistivity at 125 degree
k=2.2;                % unitless % (endwinding+active)/active region ratio 
mu0=4*pi*1e-7;        % permeability
f=50;                 % in 1/s % frequency of the current

nomexPaperThickness = 0.2e-3; % in m, paper between Roebel jumps
insulation=0.22e-3;   % in m % strand-to-strand insulation length
%% Arrays
...
% height_strand=(total_height-4*nomexPaperThickness)/(numberOfverticalStrand+1)-insulation; %corrected with below one
height_strand=(total_height-4*nomexPaperThickness)/numberOfverticalStrand-insulation; %corrected with below one
